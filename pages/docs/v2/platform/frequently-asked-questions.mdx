...
Only 10 users can request access to a team before an <InlineCode>OWNER</InlineCode> has to either approve or decline those requests. This ensures the requests cannot be abused with spam requests and that the list of users who requested access will always be manageable.

## Inviting Users to Team Who Requested Access

If a user already requested access to the team it's not possible to invite them. Instead, their request has to be accepted by an <InlineCode>OWNER</InlineCode> of the team.
